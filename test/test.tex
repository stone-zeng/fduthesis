...
% \RequirePackage{l3benchmark,xparse}
% \ExplSyntaxOn
% \NewDocumentCommand \TIMING {m} {\benchmark:n{#1}}
% \ExplSyntaxOff
...
%\setCJKmainfont{Source Han Sans SC}[ItalicFont=YouYuan]

\def\WORLDb{你好，世界。}

% \setCJKmainfont[ItalicFont=YouYuan]{Source Han Serif SC}
...
\newcommand\fonttest{%
  ...
  粗斜：\qquad\textit{\textbf{\fonttesttext}}
  % 小型大写并不需要
...
%
\mainmatter
\chapter{文本，字体，脚注 Text, font and footnote}
...
\subsection{数学}
\[ \int \sin \symbf{x} \symrm{d} \symbf{x} = \pi \Sigma ABC\]

\subsection{句号}
A: [before \verb+\fdusetup}+] \WORLDa \par
B: [after \verb+\fdusetup}+] \WORLDb  \par
C: [normal text] 你好，世界。 \par
C: [use \verb+\symbol+] 你好，世界\symbol{"3002}

\textit{\WORLDa\WORLDb 。\symbol{"3002}}

\textsf{\WORLDa\WORLDb 。\symbol{"3002}}

\textbf{\addCJKfontfeatures{Mapping = fullwidth-stop}\WORLDa\WORLDb 。\symbol{"3002}}

\texttt{\WORLDa\WORLDb 。\symbol{"3002}}

{\fontspec{Latin Modern Mono}[]``fi fl ff sh ch ck st tz !` ?` ''}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=3cm]{../logo/pdf/fudan-emblem-new-a-blue.pdf}
